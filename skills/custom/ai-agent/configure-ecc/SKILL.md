...
- User says "configure ecc", "install ecc", "setup everything Codex", or similar
...
git clone https://github.com/affaan-m/everything-Codex.git /tmp/everything-Codex
```

Set `ECC_ROOT=/tmp/everything-Codex` as the source for all subsequent copy operations.
...
| `Codex-api` | Anthropic Codex API patterns: Messages, streaming, tool use, vision, batches, Agent SDK |
