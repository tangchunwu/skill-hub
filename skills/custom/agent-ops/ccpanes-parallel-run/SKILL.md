...
description: 'CC-Panes bundled skill: 监督并行'
---

# 监督并行

拆分子任务，并行启动多个 Claude/Codex 实例执行，轮询状态并汇总结果。
...
对每个子任务调用 `ccpanes.launch_task`，记录所有 `sessionId`。
...
使用 `ccpanes.get_session_status` 检查状态（Active/Idle/Exited）。

### 5. 汇总结果

所有子任务完成后：
1. 调用 `ccpanes.get_session_output` 读取每个会话的输出
2. 汇总各子任务的执行结果
3. 标注成功/失败状态
4. 生成总结报告
