...
description: 'CC-Panes bundled skill: 工作空间管理'
---

# 工作空间管理

通过 MCP 工具管理 CC-Panes 的工作空间：创建、查看、添加项目、扫描导入。
...
调用 `ccpanes.list_workspaces`，以表格形式展示。

### `show <name>` — 查看工作空间详情

调用 `ccpanes.get_workspace`（参数: `workspaceName`）。

### `create <name> [--path <path>]` — 创建工作空间

调用 `ccpanes.create_workspace`（参数: `name`, `path`）。

### `add <workspace> <project-path>` — 添加项目

调用 `ccpanes.add_project_to_workspace`（参数: `workspaceName`, `projectPath`）。

### `scan <directory>` — 扫描并批量导入

1. 调用 `ccpanes.scan_directory`（参数: `path`）发现 Git 仓库
2. 展示发现的仓库列表
3. 询问用户是否创建工作空间并导入
4. 若确认：
...
调用 `ccpanes.list_projects`，以表格形式展示。
...
- 破坏性操作（删除工作空间/移除项目）需在 CC-Panes UI 中手动执行
- 修改会通过文件系统监控自动同步到 CC-Panes UI
