...
description: 'CC-Panes bundled skill: Todo 驱动'
---
...
对每个待分派的 Todo：
1. 调用 `ccpanes.update_todo`，状态改为 `in_progress`
2. 调用 `ccpanes.launch_task`，prompt 包含 Todo 内容
...
调用 `ccpanes.query_todos` 展示列表，不执行分派。

### `create <title> [--priority high] [--project /path]` — 创建待办

调用 `ccpanes.create_todo`。

### `done <todoId>` — 标记完成

调用 `ccpanes.update_todo`（参数: `id`, `status: "done"`）。
