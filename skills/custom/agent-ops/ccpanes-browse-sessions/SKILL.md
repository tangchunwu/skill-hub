...
description: 'CC-Panes bundled skill: 会话浏览'
---

# 会话浏览

列出 CC-Panes 中所有终端标签页的状态，读取指定标签的上下文和输出内容。
...
调用 `ccpanes.list_sessions`，以表格展示：
...
调用 `ccpanes.get_session_status`（参数: `sessionId`）。

### `read <sessionId> [lines]` — 读取会话输出

调用 `ccpanes.get_session_output`：
- `sessionId`: 目标会话
...
调用 `ccpanes.list_launch_history`：
...
调用 `ccpanes.list_claude_sessions`：
- `projectPath`（可选）: 按项目筛选
- `limit`: 默认 20
